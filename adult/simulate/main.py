...
parser.add_argument('--embedding_dim', type=int, default=12, help='dimension of embedding')
...
parser.add_argument('--embedding_num', type=int, default=100, help="the size of embedding dictionary")
...
parser.add_argument('--first_order', action='store_true', default=False, help="utilize first order or not!")
...
def main():
    ...
    if args.mode == "DSNAS":
        ...
        for search_epoch in range(1):
            g, gp, loss, rewards = train(train_queue, model, optimizer, arch_optimizer, logging)
            logging.info("total reward: %s" % rewards)
    elif args.mode == "FM_v2":
        model = FM_v2(args.embedding_dim, args.weight_decay, args)
        optimizer = torch.optim.Adagrad([param for name, param in model.named_parameters() if name != 'log_alpha'],
                                        args.lr)
        arch_optimizer = None
        for search_epoch in range(1):
